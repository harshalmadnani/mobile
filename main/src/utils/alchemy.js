  ...
  try {
    ...
    for (let token of nonZeroBalances) {
      ...
      try {
        ...
        const element = COINS.find(
          x =>
            x.id === id ||
            x.symbol.toLowerCase() === metadata.symbol.toLowerCase(),
        );
        ...
        if (
          metadata.symbol.length < 15 &&
          metadata.market !== undefined &&
          metadata.market !== null
        ) {
          data.push({
            balance: balance,
            name: metadata.name,
            ...
            price_change_percentage_24h:
              metadata.market.market_data.price_change_percentage_24h,
          });
        }
      } catch (e) {
        console.log(e);
...
export async function getBalance(mainnet, authAddress, tokens) {
  data = [];
